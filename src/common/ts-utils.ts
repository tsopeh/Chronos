export const isDefined = <T>(something: T): boolean => something !== undefined && something !== null;

export const isNotDefined = <T>(something: T): boolean => !isDefined(something);

export const documentCreateElement = (doc: Document) => <T extends HTMLElement>(tagName: string): T => doc.createElement(tagName) as T;
