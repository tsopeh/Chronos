...
const onDomMutation = (observerOfElementsInView: IntersectionObserver): MutationCallback => (
    mutations: MutationRecord[]
) => {
    mutations.forEach((mutation: MutationRecord) => {
        if (mutation.type === "childList") {
            const addedMediaElements: MediaElement[] = getMediaElementsFromNodes([
                ...mutation.addedNodes
            ]);
            ...
            const removedMediaElements: MediaElement[] = getMediaElementsFromNodes([
                ...mutation.removedNodes
            ]);
            removedMediaElements
                .map((element) => element.dataset.chronosId)
                .forEach(removeControlStrip);
...
const processAlereadyExistingMediaElements = (elementInViewObserver: IntersectionObserver) => (
